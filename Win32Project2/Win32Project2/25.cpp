...
bool testPointInRectangle(Pt2 thePoint, Pt2 pt1, Pt2 pt2, Pt2 pt3)
{
	...
	{
		bTest = false;
	//	std::cout << "testPointInRectangle():testRange()失败" << std::endl;
		return bTest;
	}
...
int getFaceIDfromthePointAndTriangleSet(Pt2 thePt, triangleSet theTriangleSet)
{
	...
	for (; iterTriangleCur != iterTriangleEnd; iterTriangleCur++)
	{
		...
		if (bTestInOrNot)
		{
			printf(" (%0.6f, %0.6f)在第%d个三角形内\n", thePt.x(), thePt.y(), faceID);
...
int main()
{
	...
	//创建网格文件
	const char * pszRasterFile = "E:\\PCLOutPut_20000B.tif";
	int bandSize = 1;
	...
	*/
	...
	for (int x = 188; x < 192; x++)
	{
		for (int y = 54; y < 58; y++ )
		{
			...
			getPointZFromDataSet(vector3Set, thisPoint, testZ, myTriangleSet);
		}
	}
