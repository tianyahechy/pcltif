...
int main()
{
	...
	PCLTif * thePCLTif = new PCLTif(xResolution, yResolution);
	//根据点云集求点云的细节
	PCLDetail theDetail = thePCLTif->getPCLDetailFromDataSet(vector3Set);
	...
	thePCLTif->getEqualXYZVectorFromDataSet(vector3Set, xSize, ySize);
	//三角化数据集合中的X,Y坐标,
	...
	//根据三角形集合计算出栅格集合，插值
	thePCLTif->getRasterVec3SetFromTriangleSet(myTriangleSet, xSize, ySize);
	time(&chazhiTime);

	rasterVec3Set imageSet = thePCLTif->getRasterVec3Set();
	//创建网格文件
	const char * pszRasterFile = "E:\\pt666000001_del5.tif";
	int bandSize = 1;
