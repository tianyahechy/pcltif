...
//从数据集合中得出指定点的高度
bool getPointZFromDataSet(pt3Set dataSet, Pt2 thePoint, double& pointZ)
{
...
//从数据集合中得出指定点的坐标XYZ
bool getPointXYZFromDataSet(pt3Set dataSet, Pt2 thePoint, Pt3& pointXYZ)
{
...
rasterVec3Set getPt3SetFromDataSetAndPt2Vector(pt3Set dataSet, rasterVec2Set Pt2Set)
{
	...
	for (; iterPt2Cur != iterPt2End; iterPt2Cur++)
	{
		...
		for (; thePt2Cur != thePt2End; thePt2Cur++)
		{
			...
			Pt3 pointXYZ(0, 0, 0);
			bool bPtInCloud = getPointXYZFromDataSet(dataSet, thePoint, pointXYZ);
			if (bPtInCloud)
			{
...
int main()
{
	...
	//得到指定点的坐标XYZ
	rasterVec3Set imageSet = getPt3SetFromDataSetAndPt2Vector(vector3Set, imagePointVector2Set);
	rasterVec3Set::iterator
	...
	//创建网格文件
	const char * pszRasterFile = "E:\\CreateTest10.tif";
	int bandSize = 1;
