...
rasterVec3Set getPt3SetFromDataSetAndPt2Vector(pt3Set dataSet, rasterVec2Set Pt2Set, triangleSet theTriangleSet, int maxz)
{
	...
	for (; iterPt2Cur != iterPt2End; iterPt2Cur++)
	{
		...
		for (; thePt2Cur != thePt2End; thePt2Cur++)
		{
			...
			if (bPtInCloud)
			{
				if ( maxz > 255.0 )
				{
					double theZ = pointXYZ.z() - 255.0;
					pointXYZ = Pt3(pointXYZ.x(), pointXYZ.y(), theZ);
				}
...
int main()
{
	...
	rasterVec3Set imageSet = getPt3SetFromDataSetAndPt2Vector(vector3Set, imagePointVector2Set,myTriangleSet, maxZ);
	rasterVec3Set::iterator
