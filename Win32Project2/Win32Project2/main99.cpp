...
int main()
{
	...
	*/
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_siftOutput2(new pcl::PointCloud<pcl::PointXYZ>);
	cloud_siftOutput2->clear();
	cloud_siftOutput2->height = cloud_siftOutput1->height;
	cloud_siftOutput2->width = cloud_siftOutput1->width;
	cloud_siftOutput2->resize(cloud_siftOutput2->width * cloud_siftOutput2->height);
	for (size_t i = 0; i < cloud_siftOutput1->size(); i++)
	{
		float x = cloud_siftOutput1->points[i].x + deltaX;
		float y = cloud_siftOutput1->points[i].y + deltaY;
		float z = cloud_siftOutput1->points[i].z + deltaZ;
		cloud_siftOutput2->points[i].x = x;
		cloud_siftOutput2->points[i].y = y;
		cloud_siftOutput2->points[i].z = z;
	}
