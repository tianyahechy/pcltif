...
int main()
{
	...
	int sizeOfColumn = 10;
	int sizeOfRow = 10;
	double distanceBetweenQuadX = distanceX / sizeOfColumn;
	double distanceBetweenQuadY = distanceY / sizeOfRow;
	quadNetWork * theNetWork = new quadNetWork(sizeOfColumn, sizeOfRow, minX, minY, distanceBetweenQuadX, distanceBetweenQuadY);

	//根据输入点集合来等分XY间距，得到图像坐标集合vector
	rasterVec2Set imagePointVector2Set = theNetWork->getEqualXYVectorFromDataSet(vector3Set, minX, minY, xSize, ySize, xResolution, yResolution);
	
	//三角化数据集合中的X,Y坐标,
	std::cout << "三角化数据集合中的X,Y坐标,重组带Z值的三角形集合" << std::endl;
	triangleSet myTriangleSet = theNetWork->getTriangleSetFromDataSet(vector3Set);
	//重组带Z值的三角形集合
	rasterVec3Set imageSet = theNetWork->getPt3SetFromDataSetAndPt2Vector(vector3Set, imagePointVector2Set, myTriangleSet);
