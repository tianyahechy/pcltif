...
PCLTif::PCLTif( PCLDetail theDetail)
{
	_rasterVecVec.clear();
...
//根据输入集合求等分插值XYZ坐标
void PCLTif::getEqualXYZVectorFromDataSet(pt3Set dataSet, double minX, double minY, int xSize, int ySize, double xResolution, double yResolution)
{
...
//根据三角形集合计算出栅格集合，插值
void PCLTif::getRasterVec3SetFromTriangleSet(triangleSet theTriangleSet, double xResolution, double yResolution, int xSize, int ySize)
{
...
//根据各个三角形计算出在该三角形外接矩形的栅格集合，插值
void PCLTif::getRasterVec3SetFromTriangle(std::vector<Pt3> theTrianglePt3, double xResolution, double yResolution, int xSize, int ySize)
{
	//1,求该三角形的外接矩形
	...
	int minRow = (minY - _theDetail.minY) / yResolution;
	int maxRow = (maxY - _theDetail.minY) / yResolution;
	int minCol = (minX - _theDetail.minX) / xResolution;
	int maxCol = (maxX - _theDetail.minX) / xResolution;
	if (maxRow >= ySize)
	{
		maxRow = ySize - 1;
	}
...
//通过栅格创建TIF文件
void PCLTif::createRasterFile(const char* strImageName, int bandSize, int xSize, int ySize, double xResolution, double yResolution, double topLeftX, double topLeftY, double rotationX, double rotationY)
{
	//设定支持中文路径
