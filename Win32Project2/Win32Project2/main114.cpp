...
//创建深度图
pcl::PointCloud<pcl::Narf36>::Ptr getNarfFromPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr inputCloud,
														float supportSize_NarfKeypoint,
														float supportSize_NarfDescriptor)
{
	Pt3 midPoint(0, 0, 0);
	double maxDistance = 0;
	double xResolution = 1.0;
	...
	double midZ = midPoint.z() + maxDistance / 5000.0;
	...
	std::cout << "深度图宽度:" << rangeImage.width << std::endl;
	std::cout << "深度图高度:" << rangeImage.height << std::endl;
	//提取深度图边界
	...
	narf_keypoint_detector.getParameters().support_size = supportSize_NarfKeypoint;  //这里最好能用近邻点数代替
	...
	narf_descriptor.getParameters().support_size = supportSize_NarfDescriptor;
...
int main()
{
	...
	double deltaX = 1000.0;			  //x方向偏移
	double deltaY = 1000.0;				  //y方向偏移
	...
	//NARF supportSize
	float supportSize_NarfKeypoint = 0.1;//1.0;
	float supportSize_NarfDescriptor = 1.0;
	...
	*/
	...
	pcl::PointCloud<pcl::Narf36>::Ptr cloud_Narf1 = getNarfFromPointCloud(cloud_sampled1, supportSize_NarfKeypoint, supportSize_NarfDescriptor);
	std::cout << "cloud_Narf1个数:" << cloud_Narf1->points.size() << std::endl;
	/*
	//估计法线
	...
	*/
	...
	pcl::PointCloud<pcl::Narf36>::Ptr cloud_Narf2 = getNarfFromPointCloud(cloud_sampled2, supportSize_NarfKeypoint, supportSize_NarfDescriptor);
	std::cout << "cloud_Narf2个数:" << cloud_Narf2->points.size() << std::endl;
	/*
	//估计法线
