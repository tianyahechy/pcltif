...
//归一化，将Z值从(minZ,maxZ)-->(0,255)
Pt3 gyh( Pt3 inputPt3, int min, int max)
{
	double x = inputPt3.x();
	double y = inputPt3.y();
	double z = inputPt3.z();
	double outputZ = (z - min) * 255.0 / (max - min);

	Pt3 outPt3 = Pt3(x, y, outputZ);
	return outPt3;
}
//指定点XY集合得出
...
rasterVec3Set getPt3SetFromDataSetAndPt2Vector(pt3Set dataSet, rasterVec2Set Pt2Set, triangleSet theTriangleSet, double minZ, double maxZ)
{
	...
	for (; iterPt2Cur != iterPt2End; iterPt2Cur++)
	{
		...
		for (; thePt2Cur != thePt2End; thePt2Cur++)
		{
			...
			{
				//归一化该值
				Pt3 ptgyh = gyh(pointXYZ, minZ,maxZ);
				theVec3.push_back(ptgyh);
			}
			else
			{
				//没在点云内就赋值为0
...
rasterVec2Set getEqualXYVectorFromDataSet(pt3Set dataSet, double & minX, double& minY, int& xnumber, int& ynumber)
{
	...
	//设定X,Y间距均为1米，
	double perXDistance = 1.0;
	double perYDistance = 1.0;

	//求X,Y方向的像素个数
	double xnumberf = distanceX / perXDistance;
	double ynumberf = distanceY / perYDistance;
...
//从集合中求出Z的最大值
double getMaxZFromPt3Set(pt3Set theVec3Set)
{
	double maxZ = 0;
	pt3Set::iterator
		iterCur = theVec3Set.begin(),
		iterEnd = theVec3Set.end();
	for (; iterCur != iterEnd; iterCur++)
	{
		double z = iterCur->second.z();
		if ( z > maxZ )
		{
			maxZ = z;
		}
...
//从集合中求出Z的最小值
double getMinZFromPt3Set(pt3Set theVec3Set)
{
	double minz = 0;
	pt3Set::iterator
		iterCur = theVec3Set.begin(),
		iterEnd = theVec3Set.end();
	minz = iterCur->second.z();
	for (; iterCur != iterEnd; iterCur++)
	{
		double z = iterCur->second.z();
		if (z <minz)
...
int main()
{
	...
	//从点云集合中求出maxZ和minz
	double maxZ = getMaxZFromPt3Set(vector3Set);
	double minZ = getMinZFromPt3Set(vector3Set);
	...
	rasterVec3Set imageSet = getPt3SetFromDataSetAndPt2Vector(vector3Set, imagePointVector2Set,myTriangleSet, minZ,maxZ);
	rasterVec3Set::iterator
	...
	//创建网格文件
	const char * pszRasterFile = "E:\\PCLOutPut_1000.tif";
	int bandSize = 1;
