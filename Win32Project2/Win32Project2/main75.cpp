...
pcl::PointCloud<pcl::PointXYZ>::Ptr getSampleCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr inputCloud, int numberOfCloudSample)
{
	...
	cloud_sampled->resize(cloud_sampled->width * cloud_sampled->height);
	cloud_sampled->clear();
	for (size_t i = 0; i < cloud_sampled->size(); i++)
	{
...
int main()
{
	...
	cloud_in_filtered_Raw->width = 100;
	...
	for (size_t i = 0; i < finalPointCloud->size(); i++)
	{
		std::cout << finalPointCloud->points[i].x << ","
			<< finalPointCloud->points[i].y << ","
			<< finalPointCloud->points[i].z << ","
			<< std::endl;
	}
