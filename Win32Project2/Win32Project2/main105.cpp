...
int main()
{
	...
	float deltaX = 1000;				//x方向平移
	float deltaY = 1000;				//y方向平移
	float deltaZ = 1000;				//z方向平移
	//SIFT关键点
	float min_scale = 0.9;
	...
	*/
	...
	for (size_t i = 0; i < correspondences->size(); i++)
	{
		...
		fprintf(fp_correspondences, "(<%d,%d>,{<%f,%f,%f>},{<%f,%f,%f>})\n", 
			firstID, secondID,
			pointX1, pointY1, pointZ1,
			pointX2, pointY2, pointZ2);
	...
	//差值点云
	pcl::PointCloud<pcl::PointXYZ>::Ptr diffPointCloud(new pcl::PointCloud<pcl::PointXYZ>);
	diffPointCloud->clear();
	diffPointCloud->height = 1;
	diffPointCloud->width = numberOfCloudSample;
	diffPointCloud->resize(diffPointCloud->width * diffPointCloud->height);
	for (size_t i = 0; i < diffPointCloud->size(); i++)
	{
		double diffX = finalPointCloud->points[i].x - cloud_sampled2->points[i].x;
		double diffY = finalPointCloud->points[i].y - cloud_sampled2->points[i].y;
		double diffZ = finalPointCloud->points[i].z - cloud_sampled2->points[i].z;
		diffPointCloud->points[i].x = diffX;
		diffPointCloud->points[i].y = diffY;
		diffPointCloud->points[i].z = diffZ;

	}
	std::string strDiffPointCloud = "E:\\test\\diffPointCloud.pcd";
	pcl::io::savePCDFile(strDiffPointCloud, *diffPointCloud);
