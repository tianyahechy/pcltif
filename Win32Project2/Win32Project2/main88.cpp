...
//根据输入点云，得到Harris2D关键点
pcl::PointCloud<pcl::PointXYZ>::Ptr getHarris2DFromCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr inputCloud )
{
	...
	pcl::PointCloud<pcl::PointXYZI>::Ptr outputCloudWithIntensity(new pcl::PointCloud<pcl::PointXYZI>);
	...
	harris.setInputCloud(cloudWithIntensity);
	harris.setNonMaxSupression(suppression);
	harris.setRefine(false);
	harris.compute(*outputCloudWithIntensity);
	

	//pcl::PointWithScale格式的点云转化为pcl::PointXYZ格式的点云
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Harris2DOutput(new pcl::PointCloud<pcl::PointXYZ>());
	cloud_Harris2DOutput->height = 1;
	cloud_Harris2DOutput->width = outputCloudWithIntensity->width;
	cloud_Harris2DOutput->is_dense = true;
	cloud_Harris2DOutput->resize(cloud_Harris2DOutput->height * cloud_Harris2DOutput->width);
	for (size_t i = 0; i < outputCloudWithIntensity->size(); i++)
	{
		double x = outputCloudWithIntensity->at(i).x;
		double y = outputCloudWithIntensity->at(i).y;
		double z = outputCloudWithIntensity->at(i).z;

		cloud_Harris2DOutput->points[i].x = x;
		cloud_Harris2DOutput->points[i].y = y;
		cloud_Harris2DOutput->points[i].z = z;

	}
	outputCloudWithIntensity->clear();
	cloudWithIntensity->clear();

	return cloud_Harris2DOutput;
}
...
int main()
{
	...
	*/
	...
	//Harris2d关键点 
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Harris2DOutput1 = getHarris2DFromCloud(cloud_sampled1);
	//std::string strcloud_siftOutput1 = "E:\\test\\cloud_siftOutput1.pcd";
	//pcl::io::savePCDFile(strcloud_siftOutput1, *cloud_siftOutput1);
	std::string strcloud_Harris2DOutput1 = "E:\\test\\cloud_Harris2DOutput1.pcd";
	pcl::io::savePCDFile(strcloud_Harris2DOutput1, *cloud_Harris2DOutput1);
	//pfh特征描述子提取
	pcl::PointCloud<pcl::PFHSignature125>::Ptr cloud_PFH1 = getPFHFromSiftAndNormalsAndCloud(cloud_sampled1,
		//cloud_siftOutput1,
		cloud_Harris2DOutput1,
		cloud_normals1,
	...
	*/
	...
	//Harris2d关键点 
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_Harris2DOutput2 = getHarris2DFromCloud(cloud_sampled2);
	//std::string strcloud_siftOutput2 = "E:\\test\\cloud_siftOutput2.pcd";
	//pcl::io::savePCDFile(strcloud_siftOutput2, *cloud_siftOutput2);
	std::string strcloud_Harris2DOutput2 = "E:\\test\\cloud_Harris2DOutput2.pcd";
	pcl::io::savePCDFile(strcloud_Harris2DOutput2, *cloud_Harris2DOutput2);
	//pfh特征描述子提取
	pcl::PointCloud<pcl::PFHSignature125>::Ptr cloud_PFH2 = getPFHFromSiftAndNormalsAndCloud(cloud_sampled2,
		//cloud_siftOutput2,
		cloud_Harris2DOutput2,
		cloud_normals2,
		numberOfNearestNeighbor);
		//pfhKDTree_SearchRadius);
	FILE *fp_cloud_PFH2;
	...
	sac.setInputSource(cloud_Harris2DOutput1);
	sac.setInputTarget(cloud_Harris2DOutput2);
	...
	trans_east.estimateRigidTransformation(*cloud_Harris2DOutput1, *cloud_Harris2DOutput2, *correspondences, transformation_matrix);
