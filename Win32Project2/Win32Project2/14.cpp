...
//通过更改栅格数据更新TIFF文件
void UpdateRasterFile(const char* strImageName, rasterVec3Set theRasterSet,double maxZ)
{
	//设定支持中文路径
	CPLSetConfigOption("GDAL_FILENAME_IS_UT8", "NO");
	//注册栅格驱动
	...
	for (; iterRasterCur != iterRasterEnd; iterRasterCur++)
	{
		...
		for (; iterCurLine != iterEndLine; iterCurLine++)
		{
			...
			if ( maxZ > 255.0)
			{
				theValue = theValue * 255.0 / maxZ;
				 //theValue = theValue * 133.0 / maxZ;
			}
			//该行逐像素赋灰度值给VECTOR
...
int main()
{
	...
	//更新网格文件,生成.tiff文件
	UpdateRasterFile(pszRasterFile, imageSet,maxZ);
	return 0;
}