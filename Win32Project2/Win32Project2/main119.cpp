...
int main()
{
	...
	*/
	...
	selectImage1Vector.resize(heightRoi1);
	for (int i = 0; i <  heightRoi1; i++)
	{
		selectImage1Vector[i].resize(widthRoi1);
		for (int j = 0; j < widthRoi1; j++)
		{
			double x = rastervecvec1[xRoi1 + i][yRoi1 + j].x();
			double y = rastervecvec1[xRoi1 + i][yRoi1 + j].y();
			double z = rastervecvec1[xRoi1 + i][yRoi1 + j].z();
			Pt3 thePt(x, y, z);
	...
	selectImage2Vector.resize(heightRoi2);
	for (int i = 0; i < heightRoi2; i++)
	{
		selectImage2Vector[i].resize(widthRoi2);
		for (int j = 0; j < widthRoi2; j++)
		{
			double x = rastervecvec1[xRoi2 + i][yRoi2 + j].x();
			double y = rastervecvec1[xRoi2 + i][yRoi2 + j].y();
			double z = rastervecvec1[xRoi2 + i][yRoi2 + j].z();
			Pt3 thePt(x, y, z);
	...
	*/
	...
	cv::Mat srcImage1(widthRoi1, heightRoi1, CV_32FC1);
	for (int i = 0; i < heightRoi1; i++)
	{
		for (int j = 0; j < widthRoi1; j++)
		{
	...
	cv::Mat srcImage2(widthRoi2, heightRoi2, CV_32FC1);
	for (int i = 0; i < heightRoi2; i++)
	{
		for (int j = 0; j < widthRoi2; j++)
		{
