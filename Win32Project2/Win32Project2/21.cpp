...
void createRasterFile(const char* strImageName, int bandSize, int xResolution, int yRosultion, double topLeftX, double topLeftY, double rotationX = 0, double rotationY = 0)
{
	...
	GDALDataset * poDS = poDriver->Create(strImageName, xResolution, yRosultion, bandSize, GDT_Float32, NULL);
	if (poDS == NULL)
	{
...
int main()
{
	...
	double maxY = theDetail.maxY;
	double topLeftX = minX;
	double topLeftY = maxY;
	rasterVec2Set imagePointVector2Set = getEqualXYVectorFromDataSet(vector3Set, topLeftX, topLeftY, xSize, ySize,xResolution,yResolution);
	...
	//创建网格文件
	const char * pszRasterFile = "E:\\PCLOutPut_2000.tif";
	int bandSize = 1;
	createRasterFile(pszRasterFile, bandSize, xResolution, yResolution, topLeftX, topLeftY);
	//更新网格文件,生成.tiff文件
